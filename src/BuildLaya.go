...
func (pOwn *sBuildLayaFile) doBuildMessageStruct(aMsg *sMessage) {
	...
	strContent += fmt.Sprintf("class TMsg%s implements ISerializable\n{\n", strMsgName)
	...
	//序列化
	strContent += "public Serialize(rBuffer: Laya.Byte): void\n{\n"
	for _, node := range aMsg.Nodes {
	...
	//反序列化
	strContent += "public Deserialize(rBuffer: Laya.Byte): void\n{\n"
	for _, node := range aMsg.Nodes {
		_, bOK := pOwn.mMapTypeRelation[node.Type]
		//基础类型
		if bOK == true {
